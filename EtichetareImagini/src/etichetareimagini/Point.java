...
public class Point implements Serializable {
	...
	public boolean near(Point pt) {
		if((Math.abs(this.getX() - pt.getX()) <= radius) && (Math.abs(this.getY() - pt.getY()) <= radius)) {
			...
			//System.out.println("Succes....X:"+Math.abs(this.getX() - pt.getX())+" Y:"+Math.abs(this.getY() - pt.getY()));
			return true;
		} else {
			//System.out.println("NU ESTE ÎNCHIS.... X:"+Math.abs(this.getX() - pt.getX())+" Y:"+Math.abs(this.getY() - pt.getY()));
			return false;
		}
	}
}
